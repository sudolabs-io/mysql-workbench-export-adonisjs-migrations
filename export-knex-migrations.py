...
# A MySQL Workbench plugin which exports all Model in the existing database to Knex migrations template
...
ModuleInfo = DefineModule(name='GenerateKnexMigration', author='Author', version='1.0.0')
migrations = {}
migration_tables = []
...
@ModuleInfo.plugin('wb.util.generate_knex_migration',
  caption='Export Knex Migration',
  input=[wbinputs.currentCatalog()],
  groups=['Catalog/Utilities', 'Menu/Catalog']
  )
@ModuleInfo.export(grt.INT, grt.classes.db_Catalog)
def generate_knex_migration(cat):
    ...
    for reference_tables in tree:
      for reference in reference_tables:
        for tbl in tables:
          ...
          for col in tbl.columns:
            ...
            try:
              ...
              if col.name == 'remember_token' and typesDict[col_type] == 'string' and str(
                ...
                # Chain collate for column types string and text
                # if typesDict[col_type] == 'string' or typesDict[col_type] == 'text' or typesDict[col_type] == 'enum':
                #   migrations[ti].append('.collate(\'utf8_unicode_ci\')')

                if col.isNotNull != 1:
                  migrations[ti].append('.nullable()')

                if col.isNotNull != 0:
                  migrations[ti].append('.notNullable()')

                if col.defaultValue != '' and col.defaultValueIsNull != 0:
                  migrations[ti].append('.defaultTo(null)')
          ...
          migrations[ti].append(migrationEndingTemplate.format(
            tableName=table_name,
            tableNameCamelCase=("".join(x.title() for x in components[0:]))
          ))
          ti += 1
  ...
  except GenerateKnexMigrationError as e:
  ...
  wizard = GenerateKnexMigrationWizard(sql_text)
  wizard.run()

  return 0


class GenerateKnexMigrationError(Exception):
...
class GenerateKnexMigrationWizardPreviewPage(WizardPage):
...
class GenerateKnexMigrationWizard(WizardForm):
  def __init__(self, sql_text):
    WizardForm.__init__(self, None)

    self.set_name('generate_knex_migration_wizard')
    self.set_title('Generate Knex Migration Wizard')

    self.preview_page = GenerateKnexMigrationWizardPreviewPage(self, sql_text)
